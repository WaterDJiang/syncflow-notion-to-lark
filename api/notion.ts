export default async function handler(req: Request): Promise<Response> {
  ...
  const path = url.pathname.replace(/^\/api\/notion/, '') + url.search;
  ...
  if (req.method === 'OPTIONS') {
    return new Response(null, {
      ...
      headers: {
        ...
        'Access-Control-Allow-Headers': 'authorization,content-type,notion-version,x-requested-with',
  ...
  respHeaders.set('Access-Control-Allow-Headers', 'authorization,content-type,notion-version,x-requested-with');
